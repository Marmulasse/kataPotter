...
public class Basket {
   ...
   public List<Discount> accumulateDiscounts(final List<Discount> discounts, final List<BookNumber> booksLeft) {
      ...
      Optional<Discount> discountOptional = Discount.fromSizeOfSeries(series.size());
      if (discountOptional.isPresent()) {
         discounts.add(discountOptional.get());
         return accumulateDiscounts(discounts, booksLeft);
      } else {
         return discounts;
      }
   }
